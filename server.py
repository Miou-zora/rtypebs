...
while True:
    ...
    tim = int(time.time())
    message = b""
    header = struct.pack(header_format, 667, tim, len(message))
    header = b"\x00" + header + message
    print("send struct", hex(0), hex(667), hex(tim), len(header), hexlify(header))
    ...
    header = struct.pack(header_format, 667, tim, len(message))
    header = b"\x01" + header + message
    print("send struct", hex(1), hex(667), hex(tim), len(header), hexlify(header))
